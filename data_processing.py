...
def load_dataset(dataset_dir, dataset_name, val_ratio, test_ratio):
    ...
    原始图
    processed_g: 由七个 dgl 图构成的元组

    process_data 函数的输出，包含用于消息传递的图以及训练、验证和测试集
    pred_pair_to_edge_labels: 字典

    键：((源节点类型, 源节点 id), (目标节点类型, 目标节点 id))
    值：字典 {规范边类型: (源节点 id 列表, 目标节点 id 列表)}
    pred_pair_to_path_labels: 字典

    键：((源节点类型, 源节点 id), (目标节点类型, 目标节点 id))
    值：列表的列表，每个列表包含 (规范边类型, 源节点 id 列表, 目标节点 id 列表)
    '''
    ...
    # print(f'pred_pair_to_edge_labels: {pred_pair_to_edge_labels.keys()}')
    # print(f'pred_pair_to_path_labels: {pred_pair_to_path_labels.keys()}')
    g = graph_list[0]
